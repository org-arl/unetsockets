...
class UnetSocket:
    ...
    remoteProtocol = 0
    timeout = -1
    provider: Optional[AgentID] = None
    waiting = None

    def __init__(self, hostname: str, port: int = 1100) -> None:
    ...
    def init(self) -> None:
        ...
        if self.gw is None:
            return
        alist: Iterable[AgentID] = self.gw.agentsForService(Services.DATAGRAM)
        for agent in alist:
    ...
    def connect(self, to: int, protocol: int) -> bool:
        ...
        send() calls.
        The default protocol number when a socket is opened is Protocol.DATA. The default
    ...
    def getTimeout(self) -> int:  # noqa: N802 - legacy API
        ...
        :returns: timeout in milliseconds, 0 for non-blocking, or -1 for infinite
        """

        return self.timeout

    def send(self, data, to: Optional[int] = None, protocol: Optional[int] = None):
        ...
        if not isinstance(data, Message):
            ...
            if to is None and protocol is None:
                ...
                if protocol != Protocol.DATA and (
                    protocol < Protocol.USER or protocol > Protocol.MAX
                ):
                    return False
        else:
            req = data
    ...
    def receive(self):
        ...
        if self.gw is None:
            return None
        t0 = time.time() * 1000
        while self.timeout <= 0 or ((time.time() * 1000) - t0) < self.timeout:
            self.waiting = True
